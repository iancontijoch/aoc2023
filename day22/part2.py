...
def compute(s: str) -> int:
    ...
    at_top = {
        k for k in overlaps
        if k not in [item for v in overlaps.values() for item in v]
    }
    in_tower = {
        item for v in overlaps.values()
        for item in v if len(v) > 1 and [item] not in overlaps.values()
    }
    unsafe_bricks = set(range(len(bricks))) - (at_top | in_tower)

    totals: dict[int, int] = defaultdict(int)
    for i in unsafe_bricks:
        overlaps_copy = deepcopy(overlaps)
        todo = deque([i])
        while todo:
            to_remove = todo.popleft()
            for k, v in overlaps_copy.items():
                if v is None:
                    continue
                if to_remove in v:
                    overlaps_copy[k].remove(to_remove)
                    if not overlaps_copy[k]:  # all supports gone, will fall
                        todo.append(k)
                        totals[i] += 1
